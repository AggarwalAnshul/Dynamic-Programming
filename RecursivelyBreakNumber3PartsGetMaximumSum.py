...
Temporal Marker: 13:59:46 Hours | Aug21, Wednesday
Temporal Marker untethered: 14:08:45 Hours | Aug21, Wednesday
LINK: ttps://www.geeksforgeeks.org/recursively-break-number-3-parts-get-maximum-sum/

T-Complexity: O(n)
S-Complexity: O(n)
"""

def findRecursivelyBreakNumber3PartsGetMaximumSum(number):
    dp = [0]*(number+1)
    ...
    for i in range(1, number+1):
        ...
        b = int(i/3)
        c = int(i/4)
        computedValue = max(a,dp[a]) + max(b, dp[b]) + max(c, dp[c]) 
        dp[i] = max(i, computedValue)
    return dp[number]

number = 1816
print(findRecursivelyBreakNumber3PartsGetMaximumSum(number))

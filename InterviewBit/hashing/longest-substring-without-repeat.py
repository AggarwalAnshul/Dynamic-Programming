...
[+]Comments                   : Too tired to optimize, will look tomorrow
                                Flashed the editorial inspired solution
                                editorial solution is essentially the same approach, the implementation is wildly consice and optimized
...
# EDITORIAL INSPIRED
# FLASHED IT
def lengthOfLongestSubstring_editorial_inspired(string):
    hashmap, length = {}, len(string)
    start, end = 0, 0
    ans = 0
    while start < length and end < length:
        if string[end] in hashmap:
            hashmap.pop(string[start])
            start += 1
        else:
